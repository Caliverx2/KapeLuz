...
import org.lewapnoob.KapeLuz.GameState
import org.lewapnoob.KapeLuz.LewapMod
import org.lewapnoob.KapeLuz.KapeLuz
...
class MyMinimapMod : LewapMod {
    private lateinit var game: KapeLuz
    ...
    override fun onEnable(game: KapeLuz) {
        this.game = game
    ...
    private fun getBlockId(game: KapeLuz, x: Int, y: Int, z: Int): Int {
    ...
    private fun getHighestBlockY(game: KapeLuz, x: Int, z: Int): Int {
        val cx = if (x >= 0) x / 16 else (x + 1) / 16 - 1
        val cz = if (z >= 0) z / 16 else (z + 1) / 16 - 1
        val chunk = game.chunks[Point(cx, cz)] ?: return 0
